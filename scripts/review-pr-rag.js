...
/**
 * Review PR with RAG Script
 * CLI entry point for RAG-enhanced PR reviews
 */
...
function printUsage() {
  console.log(`
Usage: node scripts/review-pr-rag.js <owner/repo> <pr-number> [options]
...
Examples:
  node scripts/review-pr-rag.js Git-Prime/flow-frontend-modules 436
  node scripts/review-pr-rag.js owner/repo 456 --debug
  node scripts/review-pr-rag.js owner/repo 789 --no-rag
`);
}
...
async function main() {
  ...
  // Create RAG reviewer
  try {
    logger.section(`AI Code Review Bot with RAG`);
